...
- `productid` — the Product identifier (e.g., `141`)
...
- **Product Isolation.** LBAC policies generated by Terraform bind directly to Entra ID groups. A user in the LaserPro group receives an LBAC policy of `{productid="141"}` and is structurally incapable of querying data tagged with `productid="145"` — the query engine itself enforces the boundary.
...
- **Cross-Product SRE Access.** An SRE team supporting both LaserPro and Pay2Go is granted an LBAC policy of `{productid=~"141|10k"}` and assigned RBAC Editor permissions on both product folders. This replaces the multi-stack access management burden of Pattern 005 with a single, declarative policy.
...
- An **LBAC Policy** of `{productid="141"}` bound to the Entra group.
...
- Her queries return only telemetry tagged with `productid="141"`, enforced by LBAC at query execution time.
...
- An **LBAC Policy** of `{productid=~"141|145"}`.
...
```promql
rate(http_requests_total{productid="145", region=~"us-.*"}[5m])
```
...
1. **Enforced Labels.** Every alert rule must include `productid` and `env` labels. CI validation rejects non-conforming rules at PR time.

2. **Notification Policies.** Terraform provisions specific Notification Policies in Alertmanager that route alerts based on the `productid` label.

3. **Dedicated Receivers.** Alerts for `productid="141"` are routed to LaserPro's specific PagerDuty rotation, Slack channels, and email distribution. Alerts for `productid="10k"` route to their own dedicated receivers. The two cannot intermix.

4. **Inhibition Isolation.** Inhibition rules are strictly scoped to the `productid` label, ensuring that an alert in Product A cannot inadvertently suppress alerts in Product B under any circumstance.
...
- A missing `productid` label causes CI rejection at PR time (GitOps Enforcement guardrail).
...
Pattern 006 introduces a **two-tier management flow** to address the scalability concern that a single GitOps-plus-Terraform pipeline cannot, by itself, sustain the developer-experience expectations of more than one hundred product teams. The two tiers separate **infrastructure** from **content** and apply the right tool to each.
...
#### Tier 1 — Infrastructure Layer (Terraform Cloud)

The infrastructure layer manages all resources where change frequency is low and review rigor is high. This tier preserves the GitOps-plus-Terraform pipeline established in Pattern 005 and remains mechanically identical in operation. Resources managed in this tier include:

- Grafana Cloud stack provisioning (Lending and Payment BU stacks)
- Product folders within each stack
- RBAC role assignments derived from Entra ID groups
- LBAC policies enforcing query-time isolation
- Data source configuration and credential wiring
- Alert rule definitions and Notification Policies
...
**Update cadence:** Slow — weekly or monthly. Reviewers: platform engineering, plus security where applicable.
...
- Configures Git Sync repository bindings on the Grafana side, granting product folders read and write linkage to specific GitHub paths.

#### Tier 2 — Content Layer (Grafana Git Sync)

The content layer manages dashboard authoring through Grafana's bidirectional [Git Sync](https://grafana.com/docs/grafana/latest/as-code/observability-as-code/git-sync/) feature, which synchronizes dashboard JSON files between Grafana Cloud and a designated GitHub repository in near real time. Resources managed in this tier are intentionally narrow:

- Dashboards (JSON)
- Dashboard folder content within already-provisioned product folders

The flow operates in two directions:

- **UI to Git.** A product engineer edits a dashboard in the Grafana UI and saves. Grafana commits the dashboard JSON to the configured repository — either directly to a branch or via a Pull Request, depending on the per-folder policy declared in Terraform.
- **Git to UI.** An engineer modifies a dashboard JSON file directly in GitHub. Grafana polls the repository (sixty-second default, webhook-accelerated when configured) and reflects the change in the UI within seconds.

**Update cadence:** Fast — multiple times per day per product. Reviewers: product team CODEOWNERS within the dashboard repository path.

#### Why the two tiers are separated

The split is deliberate. Infrastructure changes (RBAC, LBAC, stacks) carry security implications and benefit from strict review with multi-minute reconciliation windows. Dashboard changes (visualizations, queries, panel layouts) are iterative product-team work that suffers under the same overhead. Forcing every dashboard tweak through a fifteen-to-thirty-minute Terraform run discourages experimentation, accumulates technical debt in unused dashboards, and erodes adoption — the precise outcome Pattern 006 must avoid as it scales to support more than one hundred products.

By managing dashboards through Git Sync while preserving Terraform for security boundaries, Pattern 006 gives product teams immediate iteration loops without compromising the structural isolation guarantees that the platform exists to enforce.

#### Tier 2 enablement is itself Tier 1 work

To prevent ambiguity: the **configuration** of Git Sync repositories — which products sync from which repos, with what branches, with what authentication — is itself an infrastructure concern and is provisioned by Terraform in Tier 1. A product team cannot self-connect a repository to a Grafana folder; the platform team declares those bindings via Terraform, and Git Sync then operates within those bindings.

#### Limitations of Git Sync

Git Sync is currently in public preview for Grafana Cloud and supports only **dashboards and dashboard folder content**. Alert rules, data sources, RBAC, LBAC, service accounts, and notification policies remain Terraform-managed in Tier 1 and are out of Git Sync's scope until Grafana extends support. The platform team will validate Git Sync stability in the sandbox stack before enabling it for production workloads. Open questions on production readiness, webhook authentication behind the Finastra firewall, Entra-ID-backed authorization, and roadmap visibility on alert-rule and folder-permission sync are tracked in the [Open Items](#stakeholder-review--resolved-concerns--open-items) section and will be raised with the Grafana team.
...
Pattern 006 treats dashboards as **first-class versioned artifacts** rather than ephemeral UI state. Every dashboard in every BU stack has a corresponding JSON file in the platform GitHub repository, and every change to that JSON file flows through the Git Sync pipeline described above.

**Why Dashboard as Code**

Dashboards captured only in the Grafana UI are difficult to review, replicate across environments, audit retroactively, or restore after accidental edits. Treating dashboards as code closes all four gaps simultaneously:

- **Review.** Dashboard changes appear as standard Pull Request diffs against JSON files, enabling peer review by product team CODEOWNERS.
- **Replication.** A dashboard authored for `env=stage` can be reused for `env=prod` by changing a template variable, with the JSON serving as the canonical definition.
- **Audit.** Every change carries a Git commit author, timestamp, and message. Retroactive audits trace from the dashboard back to the engineer who modified it.
- **Recovery.** A misconfigured dashboard is reverted with a one-line Git operation rather than reconstructed by memory.

**Repository binding**

Dashboard JSON files are organized within the existing repository under each product folder (see [Repository Layout](#repository-layout)). The `dashboards/` directory under each product is the binding point. Terraform declares each `dashboards/` path as a Git Sync source for its corresponding product folder in Grafana Cloud, and from that point on the directory is bidirectionally synchronized.

**How to manage Dashboard as Code**

Product teams have two authoring workflows, both producing the same versioned outcome:

- **Workflow A — Visual editing in Grafana UI.** An engineer opens an existing dashboard or creates a new one in the Grafana UI, edits panels, queries, and layout, then saves. Grafana commits the resulting JSON to the configured repository path — either directly to `main` or as a Pull Request, depending on the per-folder policy declared in Terraform. The engineer never touches JSON directly.

- **Workflow B — Direct JSON authoring.** An engineer comfortable with the Grafana dashboard schema edits the JSON file in their IDE, opens a Pull Request against the repository, and merges after CODEOWNER approval. Grafana reflects the change in the UI within sixty seconds (or sooner with webhooks enabled).

Both workflows respect the same CODEOWNERS rules, the same review gates, and the same audit trail. The choice between them is purely ergonomic — the platform does not prefer one over the other.

**Onboarding a new dashboard**

A new dashboard is onboarded by either workflow above. There is no separate provisioning step: the product folder's Git Sync binding is already in place from Tier 1, so any new JSON file dropped into the product's `dashboards/` directory appears in the corresponding Grafana folder automatically.

**Promoting a dashboard across tiers**
...
│   │   │   ├── dashboards/      # Dashboards (Tier 2 — synced via Git Sync)
│   │   │   ├── alerts/          # Alert rules (Tier 1 — Terraform-managed)
...
Each product's `dashboards/` directory is the Tier 2 Git Sync binding point. The `alerts/` and `main.yaml` files in each product directory remain Tier 1 (Terraform-managed) artifacts.
...
**Implementation:** The SRE is added to the `GRP_Grafana_LEND_SRE` Entra security group. Terraform automatically provisions an LBAC policy of `{productid=~"141|145"}` along with RBAC Editor permissions on both product folders within the `AZR-C03-LEND-0001` stack. The SRE accesses unified dashboards correlating data across both products from a single Grafana session.
...
**Implementation:** The developer is added to `GRP_Grafana_LEND_LaserPro_Dev`. Terraform generates an LBAC policy of `{productid="141"}` along with RBAC Viewer permission scoped exclusively to the LaserPro folder. The LoanIQ folder is hidden from the developer's UI, and direct PromQL queries for LoanIQ data return zero results — the isolation is enforced at the query engine level, not by policy convention.
...
3. Provisions an LBAC policy of `{productid="newproduct"}`.
4. Configures the Alloy ingestion endpoint with enforced label injection.
5. Declares the product's `dashboards/` directory as a Git Sync source bound to the new product folder, so dashboards authored in the UI begin syncing to the repository immediately.
6. Creates default dashboard scaffolding for the team to customize.
...
| **Dashboard Iteration Loop** | Per-stack click-ops or full Terraform run | Bidirectional Git Sync — near real-time |
...
- **Reliance on Label Discipline.** Isolation and cost attribution depend entirely on the strict enforcement of ingestion labels (`productid="xyz"`). Should ingestion pipelines fail to inject these labels, data attribution will fail. This risk is mitigated by enforcing label injection at the Alloy collector level and rejecting non-conforming telemetry at ingestion.
...
- **Git Sync Preview Status and Scope.** Grafana's Git Sync feature is currently in public preview and supports only dashboards and dashboard folder content. Alert rules, data sources, and access policies remain Terraform-managed until Grafana extends the supported resource set. The platform team will validate Git Sync stability in the sandbox stack before enabling it for production product folders.
...
**Phase 2 — Update Collectors (Weeks 3 to 6, Risk: Medium).** Shift telemetry ingestion endpoints from the old per-product stacks to the new BU-aligned stacks. Alloy collectors operate in dual-write mode during this phase, sending telemetry simultaneously to both old and new stacks while enforcing label injection (`productid`, `env`, `bu`, `region`) at the edge.

**Phase 2.5 — Enable Dashboard as Code (Weeks 5 to 6, Risk: Low).** Once telemetry is flowing into the new BU-aligned stacks, enable Grafana Git Sync on a per-product basis, beginning with one pilot product per BU. The platform team declares the Git Sync repository binding in Terraform for the pilot product folders, validates bidirectional sync behavior, then expands to the remaining product folders. Dashboards authored during Phase 3 onwards land in the repository automatically rather than requiring scripted migration.
...
- **Label Taxonomy Governance.** The labeling taxonomy (`bu`, `productid`, `env`, `region`) will be documented as a strict governance artefact with examples, validation rules, and consequences for non-compliance.
...
- **Dashboard-as-Code Walkthrough.** A short hands-on guide will demonstrate both Workflow A (UI editing with Git Sync auto-commit) and Workflow B (direct JSON authoring), so each product team can adopt the workflow that fits their engineers.
...
- **Git Sync Authentication Scope.** Git Sync repository bindings are provisioned by Terraform with narrowly scoped GitHub credentials, limited to the specific product `dashboards/` paths declared in the binding. A compromised Git Sync token cannot reach alert rules, RBAC, LBAC, or any other infrastructure-tier resource.
